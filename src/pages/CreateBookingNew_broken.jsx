  ...
  const handleCreateBooking = async () => {
    ...
    try {
      ...
      const bookingData = {
        ...
        discount_percentage: bookingDetails.discount_percentage,
        discount_amount: bookingDetails.discount_amount,
        currency: 'BDT',
        auto_checkin: bookingDetails.auto_checkin,
        payment_method: paymentData.payment_method,
        paid_amount: paymentData.paid_amount,
        payment_status: paymentData.paid_amount >= finalAmount ? 'paid' : 'partial'
      };
      ...
      if (response.success) {
        ...
        setInvoiceData(response.invoice_id);
        setCurrentStep(5);
      } else {
    ...
    <div className="create-booking-page">
      ...
      <div className="booking-content">
        ...
        {currentStep === 2 && (
          ...
          {errors.room_id && <div className="error-text center">{errors.room_id}</div>}            <div className="step-actions">
              <button 
                className="btn btn-secondary"
                onClick={handleBack}
              >
            ...
            <div className="booking-confirmation">
              <div className="confirmation-card">
                ...
                <div className="confirmation-grid">
                  ...
                  <div className="confirmation-item">
                    <label>Room</label>
                    <span>Room {bookingResult.room_number} - {bookingResult.room_type}</span>
                  </div>
                  
                  <div className="confirmation-item">
                    <label>Check-in</label>
                    <span>{new Date(bookingResult.checkin_date).toLocaleDateString()}</span>
                  </div>
                  
                  <div className="confirmation-item">
                    <label>Check-out</label>
                    <span>{new Date(bookingResult.checkout_date).toLocaleDateString()}</span>
                  </div>
                  
                  {bookingDetails.discount_percentage > 0 && (
                    <div className="confirmation-item">
                      <label>Discount Applied</label>
                      <span>{bookingDetails.discount_percentage}%</span>
                    </div>
                  )}
                  
                  <div className="confirmation-item">
                    <label>Total Amount</label>
                    <span className="total-amount">৳{bookingResult.total_amount}</span>
                  </div>
                  
                  <div className="confirmation-item">
                    <label>Payment Method</label>
                    <span>{paymentData.payment_method.replace('_', ' ').toUpperCase()}</span>
                  </div>
                  
                  <div className="confirmation-item">
                    <label>Payment Status</label>
                    <span className={`status ${paymentData.payment_status}`}>
                      {paymentData.payment_status === 'paid' ? 'Paid' : 'Partial Payment'}
                    </span>
                  </div>
                  
                  <div className="confirmation-item">
                    <label>Status</label>
                    <span className={`status ${bookingResult.status}`}>
                      {bookingResult.status === 'checked_in' ? 'Checked In' : 'Confirmed'}
                    </span>
                  </div>
                  ...
                  {invoiceData && (
                    <div className="confirmation-item">
                      ...
                      <span className="invoice-number">INV-{invoiceData}</span>
                    </div>
                  )}
                </div>
              </div>
